...
class RealTimeS2SAgent:
    ...
    def convert_text_to_speech(self, text: str) -> str:
        ...
        # Post-process to remove or soften tags that cause weird sounds
        original_text = text
        text = re.sub(r'\(\w+\s*\w*\)', '', text).strip()  # Remove (big smile), (laughs), etc.
        ...
        with torch.no_grad():
            outputs = self.tts_model.generate(
                **inputs, 
                max_new_tokens=2048,
                guidance_scale=3.0, 
                temperature=0.8,  # Lower for more natural speech
                top_p=0.95, 
                top_k=50,
                do_sample=True,  # Added for variability control
...
if __name__ == "__main__":
    ...
    # 3. Launch the server.
    ui.launch(server_name="0.0.0.0", server_port=7860, share=True)  # Added share=True for public link
